...
% (11/10/91) AJCD Hacked to version 3.14.
% (15/8/92)  AJCD Hacked to version 3.141.
% 
...
@!param_size=100; {maximum number of simultaneous macro parameters}
@!nest_size=40; {maximum number of semantic levels simultaneously active}
...
@!pool_size=124000; {maximum number of characters in strings, including all
  error messages and help texts, and the names of all fonts and
  control sequences; must exceed |string_vacancies| by the total
  length of \TeX's own strings, which is currently about 24000}
...
@!trie_size=30000; {space for hyphenation patterns; should be larger for
  \.{INITEX} than it is in production versions of \TeX}
@!trie_op_size=800; {space for ``opcodes'' in the hyphenation patterns}
@!neg_trie_op_size=-800; {for lower trie_op_hash array bound}
