...
/* Modifications for AmigaDOS by David Crooke, dcc@dcs.ed.ac.uk - Feb 93 
   Principally to do with file search paths and disabling Unix-specific stuff 
   (e.g. where there is #ifdef SYS_V .... #else (assume BSD) #endif) and to
   put in the variable-memory-size hack.

   Many thanks to all responsible for TeX and associated software,
   and especially Donald Knuth and Leslie Lamport.
...
#ifdef VARMEM
static char texconfig[MAXPATHLENGTH]=TEXCONFIG;
#endif
...
#ifdef VARMEM
    copy_path(texconfig, getenv("TEXCONFIG"));
#endif
}
...
int pathspec;
{
    ...
    if ( strchr(tex_name,':') ) {	/* Absolute pathname-don't do path searching */
...
void dcc_getsizes(void)

{
  FILE *cfile;
  char cmd[256],parm[256],*ret;
  int val,index;

  strcpy (nameoffile," texmemory.config");
  if (! Openin(&cfile, configpathspec) ) return;

  while ( ret = fgets(cmd, (size_t) 255, cfile) )
  {
    if ( strncmp(cmd, "set ", 4) == 0 )
    {
      ...
      {
	index=0;
	while ( index < DCC_NP && strcmp(parm,dcc_pname[index]) ) index++;
        if ( index >= DCC_NP ) 
	{
...
#ifdef DEBUG  
  for(index = 0 ; index<DCC_NP ; index++ ) fprintf(stderr,"%d\n",dcc_pval[index]); 
#endif
}


void dcc_memerr(void)

{
  fprintf(stderr,"Insufficient memory for TeX - alter texmemory.config\n");
  exit(20);
};


void dcc_getmemory(void)

{
  if (! (strpool=(packedASCIIcode *) malloc((poolsize + 1) * sizeof(packedASCIIcode))))
    dcc_memerr();
  if (! (strstart=(poolpointer *) malloc((maxstrings + 1) * sizeof(poolpointer))))
    dcc_memerr();
  if (! (zzzaa=(memoryword *) malloc((memmax - memmin + 1) * sizeof(memoryword))))
    dcc_memerr();
#ifdef dEBUG
  if (! (zzzab=(freearr *) malloc((memmax - memmin + 1) * sizeof(freearr))))
    dcc_memerr();
  if (! (zzzac=(wasfree *) malloc((memmax - memmin + 1) * sizeof(wasfree))))
    dcc_memerr();
#endif
  if (! (savestack=(memoryword *) malloc((savesize + 1) * sizeof(memoryword))))
    dcc_memerr();
  if (! (inputstack=(instaterecord *) malloc((savesize + 1) * sizeof(instaterecord))))
    dcc_memerr();
  if (! (inputfile=(alphafile *) malloc((maxinopen + 1) * sizeof(alphafile))))
    dcc_memerr();
  if (! (linestack=(integer *) malloc((maxinopen + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (fontinfo=(memoryword *) malloc((fontmemsize + 1) * sizeof(memoryword))))
    dcc_memerr();
  if (! (fontcheck=(fourquarters *) malloc((fontmax + 1) * sizeof(fourquarters))))
    dcc_memerr();
  if (! (fontsize=(scaled *) malloc((fontmax + 1) * sizeof(scaled))))
    dcc_memerr();
  if (! (fontdsize=(scaled *) malloc((fontmax + 1) * sizeof(scaled))))
    dcc_memerr();
  if (! (fontparams=(halfword *) malloc((fontmax + 1) * sizeof(halfword))))
    dcc_memerr();
  if (! (fontname=(strnumber *) malloc((fontmax + 1) * sizeof(strnumber))))
    dcc_memerr();
  if (! (fontarea=(strnumber *) malloc((fontmax + 1) * sizeof(strnumber))))
    dcc_memerr();
  if (! (fontbc=(eightbits *) malloc((fontmax + 1) * sizeof(eightbits))))
    dcc_memerr();
  if (! (fontec=(eightbits *) malloc((fontmax + 1) * sizeof(eightbits))))
    dcc_memerr();
  if (! (fontglue=(halfword *) malloc((fontmax + 1) * sizeof(halfword))))
    dcc_memerr();
  if (! (fontused=(boolean *) malloc((fontmax + 1) * sizeof(boolean))))
    dcc_memerr();
  if (! (hyphenchar=(integer *) malloc((fontmax + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (skewchar=(integer *) malloc((fontmax + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (bcharlabel=(fontindex *) malloc((fontmax + 1) * sizeof(fontindex))))
    dcc_memerr();
  if (! (fontbchar=(short *) malloc((fontmax + 1) * sizeof(short))))
    dcc_memerr();
  if (! (fontfalsebchar=(short *) malloc((fontmax + 1) * sizeof(short))))
    dcc_memerr();
  if (! (charbase=(integer *) malloc((fontmax + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (widthbase=(integer *) malloc((fontmax + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (heightbase=(integer *) malloc((fontmax + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (depthbase=(integer *) malloc((fontmax + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (italicbase=(integer *) malloc((fontmax + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (ligkernbase=(integer *) malloc((fontmax + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (kernbase=(integer *) malloc((fontmax + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (extenbase=(integer *) malloc((fontmax + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (parambase=(integer *) malloc((fontmax + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (dvibuf=(eightbits *) malloc((dvibufsize + 1) * sizeof(eightbits))))
    dcc_memerr();
  if (! (trie=(twohalves *) malloc((triesize + 1) * sizeof(twohalves))))
    dcc_memerr();
#ifdef INITEX
  if (! (triec=(packedASCIIcode *) malloc((triesize + 1) * sizeof(packedASCIIcode))))
    dcc_memerr();
  if (! (trieo=(quarterword *) malloc((triesize + 1) * sizeof(quarterword))))
    dcc_memerr();
  if (! (triel=(triepointer *) malloc((triesize + 1) * sizeof(triepointer))))
    dcc_memerr();
  if (! (trier=(triepointer *) malloc((triesize + 1) * sizeof(triepointer))))
    dcc_memerr();
  if (! (triehash=(triepointer *) malloc((triesize + 1) * sizeof(triepointer))))
    dcc_memerr();
  if (! (trietaken=(boolean *) malloc((triesize + 1) * sizeof(boolean))))
    dcc_memerr();
#endif
  if (! (hyfdistance=(smallnumber *) malloc((trieopsize + 1) * sizeof(smallnumber))))
    dcc_memerr();
  if (! (hyfnum=(smallnumber *) malloc((trieopsize + 1) * sizeof(smallnumber))))
    dcc_memerr();
  if (! (hyfnext=(quarterword *) malloc((trieopsize + 1) * sizeof(quarterword))))
    dcc_memerr();
#ifdef INITEX
  if (! (zzzaf=(integer *) malloc((trieopsize - negtrieopsize + 1) * sizeof(integer))))
    dcc_memerr();
  if (! (trieoplang=(ASCIIcode *) malloc((trieopsize + 1) * sizeof(ASCIIcode))))
    dcc_memerr();
  if (! (trieopval=(quarterword *) malloc((trieopsize + 1) * sizeof(quarterword))))
    dcc_memerr();
#endif
  if (! (buffer=(ASCIIcode *) malloc((bufsize + 1) * sizeof(ASCIIcode))))
    dcc_memerr();
}  
