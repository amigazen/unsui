...
VOID __regargs Free_EnvVarPath(struct EnvVarPath *p)
{
	if (p)
	{
		...
		if (p->storage.buffer)
			FreeMem(p->storage.buffer, p->size);

		if (p->defpath)
			FreeMem(p->defpath, strlen(p->defpath) + 1);
 ...
 * If want to know the full filename opened we have to provide a buffer
 * and size, otherwise simply pass NULL for such arguments.
 *
*/
BPTR __regargs
