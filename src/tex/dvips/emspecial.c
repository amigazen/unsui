...
void emgraph();
	...
	if (strncmp(emp, "linewidth", 9) == 0) {
	   ...
	   sscanf(emp, "%lf%2s", &emwidth, emunit);
#else
   ...
   (void)fprintf(stderr,"em special: Linewidth temporarily set to %.1f dots\n", 
	   ...
	   if (*emp) {
#if defined(AMIGA) && defined(FLTTODBL)
	      sscanf(emp, "%lf%2s", &emwidth, emunit); /* read width */
#else
	      ...
	      if (*emp) {
#if defined(AMIGA) && defined(FLTTODBL)
	         sscanf(emp, "%lf%2s", &emheight, emunit); /* read height */
#else
 ...
 */
#ifndef AMIGA /* already defined in "emspecial_protos.h" */
typedef struct 
{
	quarterword man;
	quarterword ver;
