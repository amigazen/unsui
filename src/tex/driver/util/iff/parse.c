...
static LONG __stdargs stdio_stream(struct Hook *, struct IFFHandle *, struct IFFStreamCmd *);

static UBYTE * omodes[2] = {"r","w"};
static char  * FileBuf = NULL;
#define FILEBUFSIZE 10240


ULONG __asm HookEntry( register __a0 struct Hook * h, 
...
LONG openifile(struct ParseInfo *pi, UBYTE *filename, ULONG iffopenmode)
{
	...
	/* Syntax fuer Clipboard ist entweder 'CLIP' oder 'CLIP.1' usw. */        
	cboard = *filename == 'C' && filename[1] == 'L' && filename[2] == 'I' && filename[3] == 'P';
 	if(cboard && filename[4] == '.') unit = atoi(&filename[5]);
	...
	if (cboard)
		...
		if (!(iff->iff_Stream =
			...
			WarningStr("Clipboard open of unit %ld failed", unit);
			return(NOFILE);
			}
		InitIFFasClip(iff);
		}
		...
		/*
		 * Buffer fuer I/O setvbuf() allocieren.
		 */
	        FileBuf = AllocMem(FILEBUFSIZE, MEMF_PUBLIC);
	        if (!FileBuf)		return(CLIENT_ERROR);
		...
		if (!(iff->iff_Stream = (ULONG)
		   fopen(filename, omodes[iffopenmode & 1])))
			...
			WarningStr("%s: File open failed", filename);
			return(NOFILE);
			}
		else
		  {
		    setvbuf((FILE *)iff->iff_Stream, FileBuf, _IOFBF, FILEBUFSIZE);
		    initiffasstdio(iff);
		  }
...
struct IFFHandle *iff;
	...
	D(bug("closeifile: About to CloseIFF if open, iff=$%lx, opened=%ld\n",
			iff, pi->opened));

	if(pi->opened)	CloseIFF(iff);

	D(bug("closeifile: About to close %s, stream=$%lx\n",
			pi->clipboard ? "clipboard" : "file", iff->iff_Stream));
	if(iff->iff_Stream)
		{
		if (pi->clipboard)
		   CloseClipboard((struct ClipboardHandle *)(iff->iff_Stream));	// (hes)
		else
		  {
		     if (FileBuf) { FreeMem(FileBuf, FILEBUFSIZE); FileBuf = NULL; }
...
LONG			error = 0L;
	...
	if (!(cn = CurrentChunk (iff)))
		...
		WarningStr("Parsing error; no top chunk");
		return(NOFILE);
		}
...
long PutCk(struct IFFHandle *iff, long id, long size, void *data)
	...
	D(bug("PutCk: PushChunk of %.4s, error = %s, size = %ld\n",
		id, "(Fehler halt)" /*IFFerr(error)*/, id));
	}
    else
	{
