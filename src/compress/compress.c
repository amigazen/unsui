...
#define COMPFLAGS       "COMPFLAGS"
#define SET_IOBUF       1
#define INPUT_BUFSZ     16384
#define OUTPUT_BUFSZ    16384
...
int debug = 0;
#else
fprintf(stderr,"Usage: compress [-dfvcVr] [-b maxbits] [-B bufsize] [file ...]\n");
}
#endif /* DEBUG */
...
register int argc; char **argv;
{
    ...
    char *compflags, *cp, *rindex(), *malloc(), *getenv();

#if SET_IOBUF
    input_bufsz = INPUT_BUFSZ;
    output_bufsz= OUTPUT_BUFSZ;
#endif
    ...
    *filelist = NULL;
     ...
     * -B bufsize => input/output buffer size, must have bufsize.
     * -D => debug
    ...
    if (compflags) {
      while (*compflags == '-') {
        switch (*(++compflags)) {
        case 'B':
          if (compflags[1] < '0' || compflags[1] > '9') {
            fprintf(stderr, "Missing bufsize\n");
            Usage();
            exit(1);
          }
          input_bufsz = output_bufsz = atoi(++compflags);
          break;
        case 'b':
          if (compflags[1] < '0' || compflags[1] > '9') {
          ...
          maxbits = atoi(++compflags);
          break;
        }
        while (*compflags && (*compflags != '-')) compflags++;
      }
    }
...
#if SET_IOBUF
    input_buffer  = malloc(input_bufsz);
    output_buffer = malloc(output_bufsz);
    if (!input_buffer || !output_buffer) {
      perror("malloc");
      exit(1);
    }
    setbuffer(stdin, input_buffer, input_bufsz);
    setbuffer(stdout,output_buffer,output_bufsz);
#endif
...
#endif
		if(!quiet)
			fprintf(stderr, "%s: ", tempname); fflush(stderr);
	    }
