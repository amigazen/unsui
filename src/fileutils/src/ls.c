     ...
     char *dirname;
{
  ...
  if (explicit_arg || format_needs_stat)
	...
	/* An absolute filename has a non-leading ':' in it */
      if (name[0] && index(name + 1, ':') || dirname[0] == 0)
#else
     ...
     unsigned int mode;
{
  ...
  if (S_ISREG (mode) && indicator_style == all
      && ((use_amiga_flags && ((mode & 0x40) || !(mode & 0x2))) ||
	  (!use_amiga_flags && (mode & (S_IEXEC | S_IEXEC >> 3 | S_IEXEC >> 6)))))
#else
  if (S_ISREG (mode) && indicator_style == all
  ...
  if (indicator_style != none)
      ...
      if (S_ISREG (filetype))
	{
	  ...
	  if (indicator_style == all
	      && ((use_amiga_flags &&
		   ((f->stat.st_mode & 0x40) || !(f->stat.st_mode & 0x2))) ||
		  (!use_amiga_flags &&
