...
char link_msg[]		= "f2c.lib math=standard";
...
set_tmp_names(Void)
{
	...
	if (debugflag == 1)
		return;
#ifdef AMIGA
	k = L_tmpnam + 1; /* gives us 5 bytes more than we need */
#else
	k = strlen(tmpdir) + 16;
#endif
...
#else
#ifdef AMIGA
	c_functions = tmpnam(c_functions);
	initfname   = tmpnam(initfname);
	blkdfname   = tmpnam(blkdfname);
	p1_file     = tmpnam(p1_file);
	p1_bakfile  = tmpnam(p1_bakfile);
	sortfname   = tmpnam(sortfname);
#else
	int pid = getpid();
