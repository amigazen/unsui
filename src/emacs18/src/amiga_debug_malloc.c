...
Printf("free_sys freeing %lx, size %ld\n", old, old->size);
...
Printf("alloc_hunk got %lx (%ld)\n", scan, (char *)scan - malloc_hunk);
	  return scan + 1;
	}
      scan = scan->next;
    }
Printf("alloc_hunk failed\n");
  return 0;
}
...
Printf("free_hunk %lx (%ld), size %ld\n", old, (char *)old - malloc_hunk, old->size);
...
Printf("realloc %lx, size %ld as %ld\n", old, old->size, size);
