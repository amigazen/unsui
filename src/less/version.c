 ...
 *      -----------------------------------------------------------------
 *      Amiga ports:
 *      (Any history previous to 1.3 lost)
 *      1.3: Original? port to Amiga by Bob Leivian     ???
 *      1.3Z: Bugs fixed to run under AmigaDos 1.3 and  6/01/91  rlz
 *           2.0 on all Amiga models through A3000.
 *           Determines size of full screen workbench
 *           window, and handles whatever size it gets
 *           and whatever font.  Permits window resizing,
 *           close gadget.  Recognizes Amiga ANSI
 *           graphic rendition codes.
 *      1.4Z: Several bugs fixed.  Error handling       6/08/91  rlz
 *           much improved.  Made residentable.
 *           User-specifiable window size/placement.
 *           Recognizes international character set
 *           characters 160 - 255 (decimal).  Regular
 *           expression searches added.
 *		1.5Z: Small bug fixes and changes to compile    2/20/93  rlz
 *			 under SAS/C v. 6.2.  Redid version string
 *			 to work with AmigaDos version command.
 *			 Rewrote Amiga console IO to permit use
 *			 of original CLI or AUX: window.
 *			 Added 'w' code to window designator, to
 *			 build Less window relative to the CLI
 *			 window from which it was launched.  All
 *			 sprintf() done with exec.library RawDoFmt(),
 *			 avoiding overhead of printf().
 *		1.6Z: The "invalid flag" error message was not	7/14/93  rlz
