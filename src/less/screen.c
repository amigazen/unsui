...
/* I didn't want to port termcap for now, but there is a version
 on fish #14 that someone might want to use */
        sc_pad = "";                /* Pad string */
        sc_home = "\x1b[\x31;1H";           /* Cursor home */
        sc_addline = "\x1b[L";       /* Add line, scroll down following lines */
        sprintf(go_to_home, "\x1b[%ld;1H", nrow);
        sc_lower_left = go_to_home;     /* Cursor to last line, first column */
        sc_move = "";               /* General cursor positioning */
        sc_clear = "\x1b[H\x1b[J";  /* Clear screen */
        sc_eol_clear = "\x1b[K";     /* Clear to end of line */
        sc_s_in = "\x1b[\x37m";   /* Enter standout (highlighted) mode */
        sc_s_out = "\x1b[\x30m";         /* Exit standout mode */
        sc_u_in = "\x1b[\x34m";         /* Enter underline mode */
        sc_u_out = "\x1b[\x30m";         /* Exit underline mode */
        sc_b_in = "\x1b[\x31m";         /* Enter bold mode */
        sc_b_out = "\x1b[\x30m";       /* Exit bold mode */
        sc_it_in = "\x1b[\x33m";     /* Enter italic mode */
        sc_it_out = "\x1b[\x30m";    /* Exit italic mode */
        sc_nv_in = "\x1b[\x37m";     /* Enter inverse video mode */
        sc_nv_out = "\x1b[\x30m";    /* Exit inverse video mode */
