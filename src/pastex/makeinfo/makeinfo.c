...
#if defined (USG) || defined (VMS) || defined (_IBMR2) || defined (AMIGA)
#include <string.h>
#include <time.h>
#else /* !USG && !VMS && !_IBMR2 && !AMIGA */
#include <strings.h>
#include <sys/time.h>
#endif /* !USG && !VMS && !_IBMR2 && !AMIGA */
...
/* Non-zero means converting to AmigaGuide V34 hypertext format. */
int amiga_guide_34 = 0;


/* Non-zero means converting to AmigaGuide V39 hypertext format. */
int amiga_guide_39 = 0;

/* Non-zero means we are writing an AmigaGuide button. */

int in_amiga_guide_button = 0;

/* Non-zero means we are writing title. */

int in_amiga_guide_title = 0;

/* Number of chars we have to hide when writing titles */

int amiga_hide_title_chars = 0;


/* Prototype for attribute setting function */
void amiga_set_attributes (char *);

#define AMIGA_CODE_ON "@{b}"
#define AMIGA_CODE_OFF "@{ub}"
#define AMIGA_BOLD_ON "@{b}"
#define AMIGA_BOLD_OFF "@{ub}"
#define AMIGA_VAR_ON "@{i}"
#define AMIGA_VAR_OFF "@{ui}"
#define AMIGA_EMPH_ON "@{i}"
#define AMIGA_EMPH_OFF "@{ui}"
#define AMIGA_ITALIC_ON "@{i}"
#define AMIGA_ITALIC_OFF "@{ui}"
...
int
  cm_tex (), cm_asterisk (), cm_dots (), cm_bullet (), cm_TeX (), cm_copyright (),
  cm_code (), cm_samp (), cm_file (), cm_kbd (), cm_key (), cm_ctrl (),
  cm_var (), cm_dfn (), cm_emph (), cm_strong (), cm_cite (), cm_italic (),
  cm_bold (), cm_roman (), cm_title (), cm_w (), cm_refill (), cm_titlefont ();

int
  cm_chapter (), cm_unnumbered (), cm_appendix (), cm_top (), cm_section (),
  cm_unnumberedsec (), cm_appendixsec (), cm_subsection (), cm_unnumberedsubsec (),
  cm_appendixsubsec (), cm_subsubsection (), cm_unnumberedsubsubsec (),
  cm_appendixsubsubsec (), cm_heading (), cm_chapheading (), cm_subheading (),
  cm_subsubheading (), cm_majorheading (), cm_raisesections (), cm_lowersections ();

/* All @defxxx commands map to cm_defun (). */
int cm_defun ();

int
  cm_node (), cm_menu (), cm_xref (), cm_ref (), cm_ftable (), cm_vtable (),
  ...
  cm_bye (), cm_item (), cm_end (), cm_infoinclude (), cm_ifinfo (), cm_kindex (),
  cm_cindex (), cm_findex (), cm_pindex (), cm_vindex (), cm_tindex (),
  cm_asis (), cm_synindex (), cm_printindex (), cm_minus (), cm_footnote (),
  cm_force_abbreviated_whitespace (), cm_example (), cm_smallexample (),
  cm_lisp (), cm_format (), cm_exdent (), cm_defindex (), cm_defcodeindex (),
  cm_sc (), cm_result (), cm_expansion (), cm_equiv (), cm_print (), cm_error (),
  cm_point (), cm_today (), cm_flushleft (), cm_flushright (), cm_smalllisp (),
  cm_finalout (), cm_math (), cm_cartouche (), cm_ignore_sentence_ender ();
     ...
     char **argv;
{
  ...
  /* if --amiga option is specified, set the amiga_guide variable */
  if (amiga_guide_34)
    amiga_guide = 1;

  /* if --amiga-39 option is specified, set the amiga_guide variable */
  if (amiga_guide_39)
    amiga_guide = 1;
		...
		while (--temp > 0 && output_paragraph[temp] != '\n')
		  {
			...
			output_column = 0;
			if (amiga_guide && !strncmp ("@{", &output_paragraph[temp + current_indent],
						     2))
...
flush_output ()
{
  ...
  for (i = 0; i < output_paragraph_offset; i++)
      ...
      if (amiga_guide)
	{
	  ...
	  if (sentence_ender (UNMETA (output_paragraph[i]) + 32))
	    output_paragraph[i] = (output_paragraph[i] & 0x7f) + 32;
	}
      else
	{
	  if (output_paragraph[i] == (unsigned char) (' ' | 0x80) ||
...
enum insertion_type
{
  menu, quotation, lisp, smalllisp, example,
     ...
     int arg;
{
  ...
  if (arg == START)
    if (amiga_guide_39 && (!in_amiga_guide_button) && !no_headers)

      {
	output_column -= (amiga_guide_hidden_chars = strlen ("@{b}") + strlen ("@{ub}"));
	add_word ("@{b}*@{ub}");
      }
     ...
     int arg;
{
  ...
  if (arg == START)
    {
      if (amiga_guide && (!amiga_guide_34))
	amiga_set_attributes (AMIGA_BOLD_ON);
      else
	{
	  in_fixed_width_font++;
	  add_char ('`');
    ...
    {
      if (amiga_guide && (!amiga_guide_34))
	amiga_set_attributes (AMIGA_BOLD_OFF);
      else
	{
	  add_word ("'");
	  in_fixed_width_font--;
     ...
     int arg, start_pos, end_pos;
{
  ...
  if (!amiga_guide || amiga_guide_34)
	...
	amiga_set_attributes (AMIGA_VAR_ON);
      if (arg == END)
	amiga_set_attributes (AMIGA_VAR_OFF);
    }
}
     ...
     int arg;
{
  ...
  if (amiga_guide && (!amiga_guide_34))
    {
      if (arg == START)
	amiga_set_attributes (AMIGA_EMPH_ON);

      if (arg == END)
	amiga_set_attributes (AMIGA_EMPH_OFF);
    }
  else
    add_char ('*');
}
     ...
     int arg, start, end;
{
  ...
  if (amiga_guide && (!amiga_guide_34))
    {
      if (arg == START)
	amiga_set_attributes (AMIGA_ITALIC_ON);
      if (arg == END)
	amiga_set_attributes (AMIGA_ITALIC_OFF);
    }
}
     ...
     int with_char;
{
  ...
  in_amiga_guide_title = 1;
  execute_string ("%s\n", temp);
  in_amiga_guide_title = 0;

  ending_pos = output_position + output_paragraph_offset;
  free (temp);
  len = (ending_pos - starting_pos) - 1;

  for (i = 0; (i < len - amiga_hide_title_chars); i++)
    add_char (with_char);
  insert ('\n');
  close_paragraph ();
  amiga_hide_title_chars = 0;
  ...
  for (i = 0; i < l; i++)
      ...
      else if (amiga_guide)
	{
	  if (string[i] == '/')
	    ...
	    string[i] = '`';
	  else if (amiga_guide && !no_headers)
	    {
	      char *set_p (char *);

	      if (set_p ("amiga_convert_nodes"))
		if (string[i] == ' ')
		  string[i] = '_';
	    }
		...
		if (input_text_offset > -1)
		  {
		    ...
		    if (nodename_from_menu && !amiga_guide)
		      {
			free (next);
...
amiga_guide_convert_menu ()
{
  ...
  if (curchar () == ':')
	...
	{
	  for (i = 0; i < input_text_offset - nodestart; i++)
	    insert (' ');
	  input_text_offset++;
	}
	  ...
	  if (!ref_flag)
	    {
	      ...
	      if ((xrefstring = set_p ("xrefstring")) &&
		  (Xrefstring = set_p ("Xrefstring")))
		add_word_args ("%s ", px_ref_flag ? xrefstring : Xrefstring);
	      else
...
cm_printindex ()
{
  ...
  if ((int) index == -1)
    ...
    {
      if (amiga_guide && !no_headers && (strcmp (index_name, "cp") == 0))
	{
	  inhibit_paragraph_indentation = 1;
      ...
      if (first_footnote_this_node)
	{
	  ...
	  if (amiga_guide)
	      ...
	      if (no_headers)
		{
		  ...
		  if ((Footnotestring = set_p ("Footnotestring")))
		    execute_string ("%s-%s", Footnotestring, current_node);
		  else
	      ...
	      else
		{
		  ...
		  output_column -= (amiga_guide_hidden_chars = strlen ("@{\"\" Link \"-Footnotes\"}")
				    + strlen (current_node));
		  if ((Footnotestring = set_p ("Footnotestring")))
		    execute_string ("@w{@@@{\"%s-%s\" Link \"%s-%s\"@}}", current_node,
			      Footnotestring, current_node, Footnotestring);
		  else
...
output_pending_notes ()
{
  ...
  switch (footnote_style)
	...
	if (amiga_guide && (Footnotestring = set_p ("Footnotestring")))
	  execute_string ("@node %s-%s,,,%s\n", current_node,
			  Footnotestring, current_node);
...
void
amiga_set_attributes (char *attr_type)
{
  int after_attr = 0;
  if (in_amiga_guide_button || printing_index || !amiga_guide_39)
    return;
  ...
  if (no_headers)
	...
	output_column -= (amiga_guide_hidden_chars = 4);

      if (strcmp (attr_type, "@{b}") == 0)
	{
	  /* Write Bold on */
	  add_word ("\x1B[1m");
	}
      if (strcmp (attr_type, "@{i}") == 0)
	{
	  /* Write Italic on */
	  add_word ("\x1B[3m");
	}
      if ((strcmp (attr_type, "@{ub}") == 0) ||
	  (strcmp (attr_type, "@{ui}") == 0))
	{
	  /* Write Bold or italic off */
	  add_word ("\x1B[0m");
	}
      if (after_attr)
	output_column -= (amiga_guide_hidden_chars = 4);
      after_attr = 0;
      if (in_amiga_guide_title)
	amiga_hide_title_chars += 4;
    }
  else if (amiga_guide_39)
    {
      if (output_column > strlen (attr_type))
	output_column -= (amiga_guide_hidden_chars = strlen (attr_type));
      else
	after_attr = 1;

      if (input_text[input_text_offset - 1] == '\\')
	add_word (" ");
      /* current_indent is the current indentation */
      add_word_args ("%s", attr_type);
      ...
      if (after_attr)
	{
      ...
      if (in_amiga_guide_title)
	amiga_hide_title_chars += strlen (attr_type);
    }
}
