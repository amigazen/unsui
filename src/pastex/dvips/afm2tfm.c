...
unsigned long len ;
{
   register char *p ;
   int i ;
#if !defined(IBM6000) && !defined(AMIGA)
#if defined MSDOS || defined OS2
   extern void *malloc() ;
#else
   extern char *malloc() ;
...
case 'v': makevpl++ ;
         ...
         break ;
#if defined(AMIGA) && defined(FLTTODBL)
case 'e': if (sscanf(argv[3], "%lf", &efactor)==0 || efactor<0.01)
#else
...
case 'c':
#if defined(AMIGA) && defined(FLTTODBL)
         if (sscanf(argv[3], "%lf", &capheight)==0 || capheight<0.01)
#else
         ...
         break ;
#if defined(AMIGA) && defined(FLTTODBL)
case 's': if (sscanf(argv[3], "%lf", &slant)==0)
#else
