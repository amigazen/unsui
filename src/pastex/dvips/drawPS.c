...
static integer zPixRound(register integer, register integer);
#else
extern void cmdout();
    ...
    int invis;
{
  ...
  if (sscanf(cp, " %ld %ld %ld %ld %lf %lf ", &xc, &yc, &xrad, &yrad,
#else
...
#if defined(AMIGA) && defined(FLTTODBL)
  if (sscanf(cp, " %lf ", &inchesPerDash) != 1)
#else
...
{                               /* as exact as psdit!!! */
  ...
  if (*cp) {
	  ...
	  if (inchesPerDash < 0.0) /* dotted */ {
	      numout((integer)penSize);
#ifdef AMIGA
	      numout((integer)fabs((double)(convPS((int)-ipd) - penSize)));
#else
