...
D_S(struct InfoData,infoData);
    ...
    if (handle)
    {
        if (!lock)	/* OLSEN */
        {
            /* get the path part of the file name, so we
             * can obtain a lock on the parent directory
             */
            pathName = PathPart((STRPTR)fileName);

            /* did we get a fully qualified file name? */
            if (pathName > fileName)
            {
                /* allocate some space to copy the path name to */
                if (pathBuffer = (STRPTR)AllocVec((LONG)(pathName - fileName + 1),MEMF_ANY))
                {
                    /* copy the path name and null-terminate it */
                    CopyMem(fileName,pathBuffer,(LONG)(pathName - fileName));
                    pathBuffer[(LONG)(pathName - fileName)] = 0;

                    /* this should give us a lock on the file parent directory */
                    lock = Lock(pathBuffer,ACCESS_READ);
        ...
        if (file = AllocVec(sizeof(struct AsyncFile) + bufferSize + 15,MEMF_ANY | MEMF_PUBLIC))
        {
            file->af_File      = handle;
    ...
    if (file)
    {
        ...
        if (result >= 0)
        {
            if (!file->af_ReadMode)
            {
                ...
                if (file->af_BufferSize > file->af_BytesLeft)	/* OLSEN */
