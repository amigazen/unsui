...
#define ACTION_WINWHITE		((short)3)
...
static struct MyNewGadget NGAry[] = {
  ...
  {{0,0,0,0,NULL,NULL,GAD_white,PLACETEXT_LEFT,(void *)0L,(APTR)ACTION_WINWHITE },       125,168, 50,20 },
...
static struct Gadget *Gad_white;
...
void OpenPrefWin(void)
{
  ...
  ChangeCheckGadTags[0].ti_Data = (is_bhook);
  GT_SetGadgetAttrsA(Gad_white, PrefWin, NULL, ChangeCheckGadTags);
...
long DoPrefWin(void)
{
  ...
  while(cont && ex == 0 && PrefWin != NULL && (msg = GT_GetIMsg(PrefWin->UserPort)) != NULL) {
    ...
    switch (msg_class) {
	  ...
	  case 'G':		/* black on white */
	    new_is_bhook = !new_is_bhook;
	    ChangeCheckGadTags[0].ti_Data = (new_is_bhook);
	    GT_SetGadgetAttrsA(Gad_white, PrefWin, NULL, ChangeCheckGadTags);
	    break;
	  ...
	  case '4':		/* 4 col screen */
	    if (new_is_ownscr) {
	      ...
	      GT_SetGadgetAttrsA(Gad_4col, PrefWin, NULL, ChangeCheckGadTags);
	      ChangeDisableGadTags[0].ti_Data = !new_is_4col;
	      GT_SetGadgetAttrsA(Gad_white, PrefWin, NULL, ChangeDisableGadTags);
	    }
	    break;
	  case 'M':
...
static struct Gadget *InitGads(void)
{
    ...
    ButtomGadTags[0].ti_Data = !is_col4;	/* disable */
    Gad_white = gad = CreateGadgetA(CHECKBOX_KIND, gad, &(NG_white->new), ButtomGadTags);
    if (gad == NULL) return(NULL);
...
//  short lace;
  ...
  if (use) {
    int change_bhook = FALSE;
    
    if (is_bhook != new_is_bhook) {
      change_bhook = TRUE;
      if (new_is_bhook) set_bhook;
      else unset_bhook;
    }
    ...
    if (!new_is_ownscr) {	// die restlichen Aenderungen werden ignoriert
      ...
      if (new_is_lace != is_lace || new_is_4col != is_col4 || is_ownscr != new_is_ownscr || new_screen_mode || size_change || change_bhook) {
